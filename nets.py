...
def train_cnn_seg_model(weights=False):
    """
    Train the CNN Segmentation model.
    :param weights: If True, will load a saved model from file.
    :return: trained model saved to file.
    """
    ...
    print('CNN Model training routine: ')
    # load training data
    X_train, y_train = load_data_seg(set='train', img_rows=img_rows, img_cols=img_cols)

    # get CNN model
    if weights:
        print(' Loading saved model from file.')
        model = load_model('output/models/seg_cnn_model.h5')
    else:
        model = cnn_seg_model()
    ...
    # load test data
    X_test, y_test = load_data_seg(set='test', img_rows=img_rows, img_cols=img_cols)
    ...
    # save metrics evolution
    np.savetxt('output/metrics_evolution/seg_cnn_model_loss.csv', hist.history['loss'])
    np.savetxt('output/metrics_evolution/seg_cnn_model_acc.csv', hist.history['acc'])
    np.savetxt('output/metrics_evolution/seg_cnn_model_val_acc.csv', hist.history['val_acc'])
    np.savetxt('output/metrics_evolution/seg_cnn_model_val_loss.csv', hist.history['val_loss'])
    print('Saved metrics evolution during training to file.')
    ...
    # save model
    model.save('output/models/seg_cnn_model.h5')
    print('CNN model saved to .h5 file.')
