...
def load_mhd_data(filename):
    ...
    # convert the image to a np.ndarray & shuffle the dimensions to get axis in the order z,y,x
    ...
    # read the spacing along each dimension (pixel widths)
    spacing = np.array(list(reversed(itkimage.GetSpacing())))

    # read the images dimensions (order z,y,x)
